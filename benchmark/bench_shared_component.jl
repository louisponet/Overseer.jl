...
function bench_insertion(c, ids::Vector{Entity}, v)
    for e in ids
        c[e] = v
    end
    return c
end
function bench_access(c, ids::Vector{Entity}, v)
    for e in ids
        v += c[e].position[1]
    end
    return v
end
function bench_access_inbounds(c, ids::Vector{Entity}, v)
    @inbounds for e in ids
        v += c[e].position[1]
    end
    return v
end
function bench_delete(c, ids::Vector{Entity})
    delete!(c, ids)
    return c
end
